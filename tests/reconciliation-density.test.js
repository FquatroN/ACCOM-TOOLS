...
test("reconciliation density rules are scoped to workbench and eligible records", () => {
  ...
  assert.match(css, /\.financial-reconciliation-table th,\s*\.financial-reconciliation-table td\s*\{\s*font-size:\s*\.88rem;\s*padding:\s*\.54rem;/);
  assert.match(css, /\.financial-reconciliation-table button\s*\{\s*font-size:\s*1rem;/);
});
