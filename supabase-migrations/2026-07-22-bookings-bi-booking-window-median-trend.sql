-- Changes the independent five-year H/A comparison to a median booking-window trend.
...
as $$
  ...
  year_trend as (
    select extract(year from booking_date)::integer as booking_year,
      ...
      round(percentile_cont(0.5) within group (order by (check_in_date - booking_date)::integer)::numeric, 1) as median_days
    from public.import_fdm_bookings
  ...
  select jsonb_build_object(
    ...
    'yearTrend', coalesce((select jsonb_agg(jsonb_build_object('bookingYear', booking_year, 'ha', ha, 'medianDays', median_days) order by booking_year, ha) from year_trend), '[]'::jsonb)
  );
$$;
