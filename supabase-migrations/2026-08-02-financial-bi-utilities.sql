create or replace function public.get_bi_financial_utilities(
...
where document.document_date >= bounds.date_from
  ...
  and (nullif(btrim(coalesce(p_supplier_nif, '')), '') is null or btrim(coalesce(document.supplier_nif, '')) = btrim(p_supplier_nif))
group by
