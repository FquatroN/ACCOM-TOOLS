...
as $$
  ...
  target_years as (
    select generate_series(
      (select year_value from current_year) - 2,
  ...
  top_rows as (
    select
      ...
      t.country_label,
      b.guest_count,
      row_number() over (partition by b.chart_year order by t.total_guests desc, t.country_label) as sort_order
    from base_rows b
...
create or replace function public.guests_bi_nationality_line()
returns table(
  chart_year integer,
  country_label text,
  guest_count bigint,
