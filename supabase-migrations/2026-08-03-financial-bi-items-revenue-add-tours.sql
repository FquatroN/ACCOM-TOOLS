...
analyses(analysis) as (
  values ('nespresso'), ('nescafe_machine'), ('souvenirs'), ('tours_discovery'), ('smartbus'), ('uber')
),
months as (
...
create index if not exists import_fdm_sales_discovery_items_revenue_idx
  on public.import_fdm_sales (sale_date) include (total)
  where lower(coalesce(sale_item, '')) like '%discovery%';
create index if not exists import_fdm_sales_smartbus_items_revenue_idx
  on public.import_fdm_sales (sale_date) include (total)
  where lower(coalesce(sale_item, '')) like '%smartbus%';
create index if not exists import_fdm_sales_uber_items_revenue_idx
  on public.import_fdm_sales (sale_date) include (total)
  where lower(coalesce(sale_item, '')) like '%uber%';
create index if not exists financial_documents_tours_discovery_items_revenue_idx
  on public.financial_documents (document_date, supplier_nif) include (amount)
  where upper(regexp_replace(coalesce(supplier_nif, ''), E'\s+', '', 'g')) in ('515124141', 'PT515124141');
create index if not exists financial_documents_smartbus_items_revenue_idx
  on public.financial_documents (document_date, supplier_nif) include (amount)
  where upper(regexp_replace(coalesce(supplier_nif, ''), E'\s+', '', 'g')) in ('507007476', 'PT507007476');
create index if not exists financial_documents_uber_items_revenue_idx
  on public.financial_documents (document_date, supplier_nif) include (amount)
  where upper(regexp_replace(coalesce(supplier_nif, ''), E'\s+', '', 'g')) = 'NL852071589B01';
