...
begin
  ...
  if p_reconciliation_id is not null then select * into v_rec from financial_reconciliations where id=p_reconciliation_id and deleted_at is null; if not found then raise exception 'Reconciliation not found.'; end if; end if;
  v_offset := (p_page - 1) * p_page_size;
  with candidate_rows as (
    select s.* from (
      select d.id, d.amount, d.document_date as source_date, d.description, d.supplier_name as supplier, d.cc as account, d.category, d.payment, d.fat as document_fat from financial_documents d where p_source_type='financial_documents' and d.document_date >= date '2026-01-01' and d.fat='S'
      union all select f.id,f.amount,f.event_date,f.description,f.guest,f.account,f.category,f.invoice,'' from import_fdm_accounts f where p_source_type='import_fdm_accounts' and f.event_date >= date '2026-01-01' and (coalesce(f.invoice_flag,false) or f.category='Compras')
...
begin
  ...
  if p_action='start' then
    ...
    insert into financial_reconciliation_audit(reconciliation_id,action,actor,metadata) values(v_id,'start',p_actor,jsonb_build_object('sourceType',p_source_type,'sourceId',p_source_id));
  else
    ...
    if p_action in ('add_item','remove_item') and r.status <> 'started' then raise exception 'Only started reconciliations can be edited.'; end if;
    ...
    elsif p_action='delete' then update financial_reconciliations set deleted_by=p_actor,deleted_at=timezone('utc',now()),updated_at=timezone('utc',now()) where id=v_id; delete from financial_reconciliation_items where reconciliation_id=v_id;
    end if;
    insert into financial_reconciliation_audit(reconciliation_id,action,actor,comment,metadata) values(v_id,p_action,p_actor,nullif(trim(coalesce(p_comment,'')),''),jsonb_build_object('sourceType',p_source_type,'sourceId',p_source_id));
  end if;
  update financial_reconciliations set difference_amount=financial_reconciliation_difference(base_source_type,matching_source_types,id),updated_at=timezone('utc',now()) where id=v_id and deleted_at is null;
  return get_financial_reconciliation_workspace(v_id, coalesce(p_source_type,(select base_source_type from financial_reconciliations where id=v_id)), coalesce(p_matching_source_types,array[]::text[]));
end $$;
...
grant execute on function public.financial_reconciliation_action(text,text,uuid,text,text[],text,uuid,text) to authenticated, service_role;
