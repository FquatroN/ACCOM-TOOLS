create or replace function public.get_bi_financial_bank_statement(
...
select
  ...
  avg(statement.montante)::numeric as average_amount,
  min(statement.montante)::numeric as min_amount,
  max(statement.montante)::numeric as max_amount,
...
create or replace function public.get_bi_financial_bank_statement_payload(
  p_year_from integer default extract(year from current_date)::integer - 10,
  p_year_to integer default extract(year from current_date)::integer
)
...
create index if not exists import_cgd_extrato_ordem_bank_statement_cover_idx
