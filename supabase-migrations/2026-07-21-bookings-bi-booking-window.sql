-- Booking-window dashboard: check-in date minus booking date, grouped by booking-date year.
...
as $$
  with filtered as (
    ...
    where booking_date is not null
      ...
      and extract(year from booking_date)::integer = p_year
      and check_in_date >= booking_date
      and (
        nullif(btrim(coalesce(p_ha, '')), '') is null
        or case when coalesce(room_type, '') ilike '%Cruz%' then 'A' else 'H' end = upper(btrim(p_ha))
      )
      and (
        coalesce(array_length(p_statuses, 1), 0) = 0
        or coalesce(nullif(btrim(status), ''), 'Unknown') = any(p_statuses)
      )
  ),
  summary as (
    select
  ...
  distribution as (
    select
      case
        when booking_window_days = 0 then 'Same day'
        when booking_window_days <= 3 then '1–3 days'
        when booking_window_days <= 7 then '4–7 days'
        when booking_window_days <= 14 then '8–14 days'
        when booking_window_days <= 30 then '15–30 days'
        when booking_window_days <= 60 then '31–60 days'
        when booking_window_days <= 90 then '61–90 days'
        else '91+ days'
      end as label,
      case
        when booking_window_days = 0 then 1
        when booking_window_days <= 3 then 2
        when booking_window_days <= 7 then 3
        when booking_window_days <= 14 then 4
        when booking_window_days <= 30 then 5
        when booking_window_days <= 60 then 6
        when booking_window_days <= 90 then 7
        else 8
      end as sort_order,
      count(*)::bigint as booking_count
    from filtered
  ...
  months as (
    select
      to_char(date_trunc('month', booking_date), 'YYYY-MM') as booking_month,
      count(*)::bigint as booking_count,
      round(avg(booking_window_days)::numeric, 1) as average_days,
      round(percentile_cont(0.5) within group (order by booking_window_days)::numeric, 1) as median_days
    from filtered
  ...
  years as (
    select distinct extract(year from booking_date)::integer as booking_year
    from public.import_fdm_bookings
    where booking_date is not null
  )
...
grant execute on function public.bookings_bi_booking_window(integer, text, text[]) to anon, authenticated, service_role;
