...
function normalizeRows(rows) {
  return (Array.isArray(rows) ? rows : []).map((row) => ({
    ...
    totalWithdrawals: Number(row?.total_withdrawals || row?.totalWithdrawals || 0),
    totalDeposits: Number(row?.total_deposits || row?.totalDeposits || 0),
    naoFaturado: Number(row?.nao_faturado || row?.naoFaturado || 0),
...
async function loadRows(filters) {
  ...
  const load = restQuery("rpc/get_bi_financial_cash_analysis_payload", {
    method: "POST",
    body: { p_year_from: filters.yearFrom, p_year_to: filters.yearTo },
  }).then((payload) => {
    const first = Array.isArray(payload) ? payload[0] : payload;
    if (!first || !Array.isArray(first.rows)) {
      const error = new Error("Cash Analysis returned an invalid aggregate payload. Run the database migration.");
...
module.exports = async function handler(req, res) {
  try {
    ...
    console.error("[financial-bi-cash-analysis] load failed", { message: error.message, statusCode: error.statusCode });
    sendError(res, error);
  }
};
