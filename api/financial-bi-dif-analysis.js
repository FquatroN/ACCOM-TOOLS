...
function normalizeRows(rows) {
  return (Array.isArray(rows) ? rows : []).map((row) => {
    const fdmAmount = Number(row?.fdm_amount ?? row?.fdmAmount ?? 0);
    const extratoAmount = Number(row?.extrato_amount ?? row?.extratoAmount ?? 0);
    return {
      analysis: clean(row?.analysis).toLowerCase(),
...
async function loadRows(filters) {
  ...
  const load = restQuery("rpc/get_bi_financial_dif_analysis_payload", {
    method: "POST",
    body: { p_year_from: filters.yearFrom, p_year_to: filters.yearTo },
  }).then((payload) => {
    const first = Array.isArray(payload) ? payload[0] : payload;
    if (!first || !Array.isArray(first.rows)) {
      const error = new Error("Dif Analysis returned an invalid aggregate payload. Run the database migration.");
...
module.exports = async function handler(req, res) {
  try {
    ...
    console.error("[financial-bi-dif-analysis] load failed", { message: error.message, statusCode: error.statusCode });
    sendError(res, error);
  }
};
